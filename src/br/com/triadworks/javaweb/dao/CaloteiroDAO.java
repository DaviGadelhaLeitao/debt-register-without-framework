...
public class CaloteiroDAO {
	...
	// ALTERA
	// preparo string sql
	// jogo essa string dentro de um PrepareStatement para ser executada no banco
	// seto nesse statement (stmt) os dados da string
	public void altera(Caloteiro caloteiro) {
		...
		try {
			...
			// o id deve nos assegurar qual caloteiro será alterado com certeza, sim a string sql nos garante isso
	...
	public void deleta(Caloteiro caloteiro) {
		...
		try {
			PreparedStatement stmt = conexao.prepareStatement(sql);
			// vai ser passado um objeto caloteiro pra esse método e ele vai pegar o ID desse objeto e
			// deletar esse objeto da tabela.
